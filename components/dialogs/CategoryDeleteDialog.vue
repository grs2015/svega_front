<template>
  <v-dialog
    persistent
    :value="true"
    max-width="500"
  >
    <v-card>
      <v-card-title class="text-h5">Delete Category?</v-card-title>
      <v-card-text>Are you sure you want to delete this category?</v-card-text>
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn
          text
          @click="$emit('close')"
        >No</v-btn>
        <v-btn
          text
          color="red darken-1"
          @click="deleteCategory"
        >Yes</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>

<script>
export default {
  props: {
    category: {
      type: Object,
      required: true
    }
  },
  methods: {
    deleteCategory() {
      const categoryData = this.category.identifier
      this.$store.dispatch('category/deleteCategory', categoryData)
        .then(() => this.$store.dispatch('showSnackBar', { message: 'The Category has been deleted!', status: 'success'}))
        .catch(e => this.$store.dispatch('showSnackBar', { message: e.response.data.error, status: 'error'}))
        .then(() => this.$emit('close'))
    }
  }
}
</script>
